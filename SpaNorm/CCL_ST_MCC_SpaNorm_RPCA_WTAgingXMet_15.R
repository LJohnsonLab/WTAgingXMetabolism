########################################################################
# Name: CCL_ST_MCC_SpaNorm_RPCA_WTAgingXMet_15.R
...
#         - 15. making UMAPs, imgdimplots, and calculating cell proportions btwn groups for immune vasc celltypes  
# Input Files: 
# Final Output Files: 
...
######### Paths ############
input_obj <- "/mnt/gpfs3_amd/pscratch/lajo247_uksr/WTAgingXMet/output/20260816_SpaNorm_RPCA_immunevasc_annotated.qs2"
...
Idents(SpaNorm_RPCA.obj) <- "immune_vasc_ann"

SpaNorm_RPCA.obj <- subset(SpaNorm_RPCA.obj, idents = "T cells", invert = T)

# cluster palettes ----
## immune_vasc_ann palette ----
col_palette <- c("Endothelial cells" = "#FDBF6F",
                    "Pericytes" = "#C45E00",
                    "Astrocytes" = "#E64B35",
                    "VLMC" = "#FF7F00",
                    "Microglia" = "#00897B",
...
# SANITY CHECK
levels(SpaNorm_RPCA.obj$immune_vasc_ann)
#[1] "Endothelial cells" "Astrocytes"        "Microglia"
# [4] "VLMC"              "BAM"               "VSMC"
# [7] "Pericytes"         "T cells"
 
 

table(SpaNorm_RPCA.obj$immune_vasc_ann)
#Endothelial cells        Astrocytes         Microglia              VLMC
#          20426             35998              9143              1939
#           BAM              VSMC         Pericytes           T cells
#          1293              5414              4813               176
...
#16wk_F1       0    3322       0       0       0       0       0
#16wk_F2       0       0       0       0       0       0    3349
#16wk_F4       0       0       0       0       0    3286       0
#16wk_M1    3443       0       0       0       0       0       0
#16wk_M2       0    3771       0       0       0       0       0
#16wk_M4       0       0       0       0    2766       0       0
#36wk_F1    3323       0       0       0       0       0       0
#36wk_F3       0       0       0       0       0    3337       0
#36wk_F4       0       0       0       0       0       0    3435
#36wk_M1       0    3179       0       0       0       0       0
#36wk_M2    3816       0       0       0       0       0       0
#36wk_M4       0       0       0       0       0       0    2763
#59wk_F2       0       0       0    4450       0       0       0
#59wk_F3       0       0       0       0       0       0    3204
#59wk_F5       0       0       0       0    2225       0       0
#59wk_M1       0       0       0    3261       0       0       0
#59wk_M3       0       0       0       0       0    3122       0
#59wk_M5       0       0       0       0       0       0    3504
#92wk_F2       0       0       0    3562       0       0       0
#92wk_F3       0       0       0       0    3001       0       0
#92wk_F5       0       0       0       0       0       0    3554
#92wk_M1       0       0    2350       0       0       0       0
#92wk_M2       0       0       0    3678       0       0       0
#92wk_M3       0       0       0       0    3501       0       0
...
# save figs 
ggsave(filename = paste0(date, "immunevasc_fov7_24_imgdim.png"), plot = fov7_24_imgdim, path = fig_path, height = 8, width = 11, dpi = 600)
ggsave(filename = paste0(date, "immunevasc_UMAP.png"), plot = gen_UMAP, path = fig_path, height = 8, width = 11, dpi = 600)
ggsave(filename = paste0(date, "immunevasc_fov7_24_imgdim_split.png"), plot = fov7_24_imgdim_split, path = fig_path, height = 8, width = 11, dpi = 600)

# Propeller: cell proportion stats btwn age groups ----
## gen propeller stats ----
gen_propeller_results <- propeller(
  x        = SpaNorm_RPCA.obj,
  clusters = SpaNorm_RPCA.obj$immune_vasc_ann,
...
write_csv(gen_propeller_results, paste0(cellprops_outputpath, date, "immunevasc_props_stats.csv"))
...
qs_save(gen_props, paste0(cellprops_outputpath, date, "immunevasc_props.qs2"))
...
celltype_zscore_results <- map(names(met_path_table), function(pathway_name) {
  ...
  celltypes <- SpaNorm_RPCA.obj$immune_vasc_ann
...
qs_save(SpaNorm_RPCA.obj, paste0(output_path, date, "SpaNorm_RPCA_immunevasc_scored.qs2"))
...
zscore_metadata <- SpaNorm_RPCA.obj@meta.data %>%
  select(immune_vasc_ann, Age, sample_ID, all_of(zscore_cols))

# save ALL zscore (global and cell specific) metadata to load locally
qs_save(zscore_metadata, paste0(output_path, date, "SpaNorm_RPCA_immunevasc_zscore_metadata.qs2"))
...
meanexp_metadata <- SpaNorm_RPCA.obj@meta.data %>%
  select(immune_vasc_ann, Age, sample_ID, all_of(meanexp_cols))

# save ALL zscore (global and cell specific) metadata to load locally
qs_save(meanexp_metadata, paste0(output_path, date, "SpaNorm_RPCA_immunevasc_meanexp_metadata.qs2"))
